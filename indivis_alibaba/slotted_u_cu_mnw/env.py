...
class Abs_utilitarian_slotted_env:
    def __init__(self, input_dict, arrival_log, execution_log, departure_log):
        ...
        task_event_folder = os.path.join(self.scratch_path, 'alibaba_trace', 'input')
        ...
        self.time_next_slot = (self.time_next_arrival + 1)
    ...
    def save_finished_log(self):
        df_int_log = pd.DataFrame(columns=['job_id', 'int_id',
                                           ...
                                           'int_completion_time'])
        if self.is_google:
            cols = ['job_id', 'num_int', 'per_int_duration', 'job_arrival_time',
                    'job_completion_time', 'req_cpu_per_int', 'req_mem_per_int',
                    'req_bw_per_int', 'mice', 'av_ef_satsf', 'av_si_satsf',
                    'boost']
        else:
            cols = ['job_id', 'num_int', 'per_int_duration', 'job_arrival_time',
                    'job_completion_time', 'req_cpu_per_int', 'req_mem_per_int',
                    'req_gpu_per_int', 'req_bw_per_int', 'mice', 'av_ef_satsf',
                    'av_si_satsf', 'boost']

        df_job_log = pd.DataFrame(columns=cols)
        for job_id, job in self.finished_log.finished_job_dct.items():
            ...
            if self.is_google:
                ...
                req_gpu_per_int = job.demanded_res[2]
                req_bw_per_int = job.demanded_res[3]
            ...
            tmp_dct = {'job_id': job_id, 'num_int': num_task,
            ...
            if self.is_google: tmp_dct['req_gpu_per_int'] = req_gpu_per_int
            df_job_log = df_job_log.append(tmp_dct, ignore_index=True)
        ...
        while (self.pointer <= self.num_task_event_rows and
            ...
            if self.is_google:
                arrived_lst += [[job_ID, inst_id, self.time_current, required_CPU,
                                 required_MEM, required_BW, single_task_execution_duration,
                                 mice, sta_t, sta_jct] for inst_id in range(int(num_inst))]
            else:
                required_GPU = self.task_event_df.loc[self.pointer - 1, 'req_gpu']
                arrived_lst += [[job_ID, inst_id, self.time_current, required_CPU,
                                 required_MEM, required_GPU, required_BW,
                                 single_task_execution_duration,
                                 mice, sta_t, sta_jct] for inst_id in range(int(num_inst))]

            self.pointer += 1
        if self.is_google:
            cols = ['job_id', 'int_id', 'int_sub_time', 'req_cpu_core', 'req_mem_GB',
                    'req_bw', 'int_runtime_sec', 'mice', 'sta_t', 'sta_jct']
        else:
            cols = ['job_id', 'int_id', 'int_sub_time', 'req_cpu_core', 'req_mem_GB',
                    'req_gpu', 'req_bw', 'int_runtime_sec', 'mice', 'sta_t', 'sta_jct']
        arrived_job = pd.DataFrame(data=arrived_lst, columns=cols)
    ...
    def allocate_from_waiting_job_dct(self, best_fit=True):
        # We find the relaxed solution and round it down
        # Based on rounded solution, we allocate task
        job_id_list, x_round = self.waiting_log.\
            allocate_from_waiting_log(self.c, self.extr_res)
        avail_comp_copy = np.copy(self.avail_comp_res)
        avail_extr_copy = np.copy(self.avail_extr_res)
        if np.sum(x_round) == 0:
            for pos, job_id in enumerate(job_id_list):
                job = self.waiting_log.waiting_job_dct[job_id]
                for k in range(len(job.waiting_task_dct)):
                    for s in range(self.avail_comp_res.shape[0]):
                        feas = self.waiting_log.\
                            is_feasible_on_mach(s, avail_comp_copy,
                                                avail_extr_copy,
                                                job.demanded_res)
                        if feas:
                            x_round[pos, s] += 1
                            avail_comp_copy[s, :] = avail_comp_copy[s, :] - np.array(job.demanded_res[0:-1])
                            avail_extr_copy = avail_extr_copy - job.demanded_res[-1]
                            break
        for pos, job_id in enumerate(job_id_list):
            num_alloc_tasks = np.sum(x_round[pos, :])
            if num_alloc_tasks == 0: continue
            self.can_allocate = True
            self.waiting_log.picked_job_id = job_id
            job = self.waiting_log.waiting_job_dct[job_id]
            self.running_log.running_job_dct[job_id] = job
            for t in range(num_alloc_tasks):
                if len(job.task_idx_sorted_by_executed_time) == 0: continue
                temp = job.task_idx_sorted_by_executed_time.pop(-1)
                self.picked_task_executed_time = temp[0]
                self.waiting_log.picked_task_idx = temp[1]
                task = job.waiting_task_dct[self.waiting_log.picked_task_idx]
                self.waiting_log.picked_server =\
                    (x_round[pos, :] != 0).argmax()
                x_round[pos, self.waiting_log.picked_server] -= 1
                self.waiting_log.picked_task_arrival_time =\
                    task.task_arrival_time
                self.waiting_log.picked_job_over = (len(job.waiting_task_dct) == 1)
    ...
    def run(self):
        while self.sim_not_over():
            if self.arrival_event():
                # read from df and create and add jobs (and their tasks)
                # to env.waiting_log.waiting_job_dct
                # In this type of allocation rules (i.e Util, CUtil, MNW) we
                # do not check the jobs in waiting_lof to allocate tasks
                # Tasks allocation happens just at each slot
                self.read_from_task_event_df()
                self.sth_arrived_or_finished_bw_slot = True
                self.time_next_arrival = \
                    self.task_event_df.loc[self.pointer - 1, 'job_sub_time'] if self.task_event_df.shape[0] >= self.pointer else np.inf
                if self.time_next_finish == np.Infinity:
                    self.time_next_slot = self.time_current
                ...
                if not self.sth_arrived_or_finished_bw_slot:
                    if (self.time_next_arrival == np.inf and
                        ...
                        break
                    k1 = (min(self.time_next_arrival, self.time_next_finish)
                          - self.time_current) // self.slot_duration
                    ...
                    # Stop any running task. apply the over_head
                    # Place all jobs to env.waiting_log.waiting_job_dct
...
class Slotted_mnw_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        ...
        self.event_log_path = os.path.join(self.RW_path2, self.name_prefix + '_event_log_mnw.csv')
        self.job_log_path = os.path.join(self.RW_path2, self.name_prefix + '_job_log_mnw.csv')
        self.task_log_path = os.path.join(self.RW_path2, self.name_prefix + '_task_log_mnw.csv')


class Slotted_utilitarian_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        super().__init__(input_dict, arrival_tracker, execution_tracker, departure_tracker)
        self.event_log_path = os.path.join(self.RW_path2, self.name_prefix + '_event_log_u.csv')
        self.job_log_path = os.path.join(self.RW_path2, self.name_prefix + '_job_log_u.csv')
        self.task_log_path = os.path.join(self.RW_path2, self.name_prefix + '_task_log_u.csv')


class Slotted_relative_utilitarian_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        super().__init__(input_dict, arrival_tracker, execution_tracker, departure_tracker)
        self.event_log_path = os.path.join(self.RW_path2, self.name_prefix + '_event_log_ru.csv')
        self.job_log_path = os.path.join(self.RW_path2, self.name_prefix + '_job_log_ru.csv')
        self.task_log_path = os.path.join(self.RW_path2, self.name_prefix + '_task_log_ru.csv')


class Slotted_cautious_utilitarian_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        super().__init__(input_dict, arrival_tracker, execution_tracker, departure_tracker)
        self.event_log_path = os.path.join(self.RW_path2, self.name_prefix + '_event_log_cu.csv')
        self.job_log_path = os.path.join(self.RW_path2, self.name_prefix + '_job_log_cu.csv')
        self.task_log_path = os.path.join(self.RW_path2, self.name_prefix + '_task_log_cu.csv')


class Slotted_cautious_relative_utilitarian_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        super().__init__(input_dict, arrival_tracker, execution_tracker, departure_tracker)
        self.event_log_path = os.path.join(self.RW_path2, self.name_prefix + '_event_log_cru.csv')
        self.job_log_path = os.path.join(self.RW_path2, self.name_prefix + '_job_log_cru.csv')
        self.task_log_path = os.path.join(self.RW_path2, self.name_prefix + '_task_log_cru.csv')

...
if __name__ == "__main__":
    ...
    # rho_list = [5, 7, 9]
    # nu_list = [40, 50, 60]
    # slot_dur_list = [16.0, 64.0, 256.0]
    bw_list = [4]
    rho_list = [9.9]
    nu_list = [int(sys.argv[5])]
    slot_dur_list = [64.0]
...
#
    ...
    j_l = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'google_micro_indivis_ef'))
    if len(sys.argv) == 4:
    ...
    elif sys.argv[-1] in ['large_cpu', 'large_mem', 'no_large']:
        fn = f"_seed{seed_start}_N{int(sys.argv[5])}_LargePC{int(sys.argv[4])}.csv"
        boost.df.to_csv(os.path.join(j_l, f"{sys.argv[-1]}_indivis_boost" + fn))
        event_df.to_csv(os.path.join(j_l, f"{sys.argv[-1]}_indivis_event" + fn))
    else:
        fn = f"_seed{seed_start}_N{int(sys.argv[5])}_LargePC{int(sys.argv[4])}.csv"
        boost.df.to_csv(os.path.join(j_l, "indivis_boost" + fn))
        event_df.to_csv(os.path.join(j_l, "indivis_event" + fn))

...
class Abs_utilitarian_slotted_env:
    def __init__(self, input_dict, arrival_log, execution_log, departure_log):
        ...
        self.avail_extr_res = np.copy(self.extr_res)
        ...
        if os.popen('hostname').read()[0:-1] in ['erfan-Inspiron-3670',
            ...
            self.trace_path = os.path.join(input_dict['scr_path'], 'alibaba_trace')
        ...
        self.RW_path = os.path.join(self.scratch_path, 'alibaba_indivis',
                                    sim_folder_n)
#         if not os.path.isdir(self.RW_path):
#             os.makedirs(self.RW_path)
#         if not os.path.isdir(os.path.join(self.RW_path, 'concat')):
#             os.makedirs(os.path.join(self.RW_path, 'concat'))
#         if not os.path.isdir(os.path.join(self.RW_path, 'fig')):
#             os.makedirs(os.path.join(self.RW_path, 'fig'))
        self.event_log_path = os.path.join(self.RW_path, 'event_log.csv')
        self.job_log_path = os.path.join(self.RW_path, 'job_log.csv')
        self.task_log_path = os.path.join(self.RW_path, 'task_log.csv')
            ...
            for job_id_2 in job_set:
                ...
                if job_id_2 not in self.running_log.running_job_dct:
                    ...
                    if np.sum([x for x in num_task_in_s.values()]) > 0:
                        ...
                        num_task_with_2 = 0
                    num_task_with_2 = min(num_task_with_2, total_num_task)
    ...
    def run(self):
        ...
        while self.sim_not_over():
            if self.arrival_event():
                ...
                num_task_dct[self.time_current] = {j_id: len(j.running_task_dct) for j_id, j in self.running_log.running_job_dct.items()}
                num_task_dct[self.time_current]['event'] = 0
                num_task_dct[self.time_current]['avail_extr_res'] = self.avail_extr_res
                num_task_dct[self.time_current]['avail_comp_res'] = np.copy(self.avail_comp_res)
                ...
                self.update_df_event()
                num_task_dct[self.time_current] = {j_id: len(j.running_task_dct) for j_id, j in self.running_log.running_job_dct.items()}
                num_task_dct[self.time_current]['event'] = 1
                num_task_dct[self.time_current]['dep_lst'] = departing_j_t_list
                num_task_dct[self.time_current]['avail_extr_res'] = self.avail_extr_res
                num_task_dct[self.time_current]['avail_comp_res'] = np.copy(self.avail_comp_res)
                self.time_current = self.time_next_event
                ...
                if not self.sth_arrived_or_finished_bw_slot:
                    ...
                    self.sth_arrived_or_finished_bw_slot = False
                    num_task_dct[self.time_current] = {j_id: len(j.running_task_dct) for j_id, j in self.running_log.running_job_dct.items()}
                    num_task_dct[self.time_current]['event'] = 2
                    num_task_dct[self.time_current]['avail_extr_res'] = self.avail_extr_res
                    num_task_dct[self.time_current]['avail_comp_res'] = np.copy(self.avail_comp_res)
                    self.time_current = self.time_next_event
        return num_task_dct
...
class Slotted_mnw_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        ...
        self.event_log_path = os.path.join(self.RW_path, 'event_log_mnw.csv')
        self.job_log_path = os.path.join(self.RW_path, 'job_log_mnw.csv')
        self.task_log_path = os.path.join(self.RW_path, 'task_log_mnw.csv')


class Slotted_utilitarian_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        super().__init__(input_dict, arrival_tracker, execution_tracker, departure_tracker)
        self.event_log_path = os.path.join(self.RW_path, 'event_log_u.csv')
        self.job_log_path = os.path.join(self.RW_path, 'job_log_u.csv')
        self.task_log_path = os.path.join(self.RW_path, 'task_log_u.csv')


class Slotted_relative_utilitarian_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        super().__init__(input_dict, arrival_tracker, execution_tracker, departure_tracker)
        self.event_log_path = os.path.join(self.RW_path, 'event_log_ru.csv')
        self.job_log_path = os.path.join(self.RW_path, 'job_log_ru.csv')
        self.task_log_path = os.path.join(self.RW_path, 'task_log_ru.csv')


class Slotted_cautious_utilitarian_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        super().__init__(input_dict, arrival_tracker, execution_tracker, departure_tracker)
        self.event_log_path = os.path.join(self.RW_path, 'event_log_cu.csv')
        self.job_log_path = os.path.join(self.RW_path, 'job_log_cu.csv')
        self.task_log_path = os.path.join(self.RW_path, 'task_log_cu.csv')


class Slotted_cautious_relative_utilitarian_env(Abs_utilitarian_slotted_env):
    def __init__(self, input_dict, arrival_tracker, execution_tracker, departure_tracker):
        super().__init__(input_dict, arrival_tracker, execution_tracker, departure_tracker)
        self.event_log_path = os.path.join(self.RW_path, 'event_log_cru.csv')
        self.job_log_path = os.path.join(self.RW_path, 'job_log_cru.csv')
        self.task_log_path = os.path.join(self.RW_path, 'task_log_cru.csv')
